  ...
  return (
    <section id="endeks" className="rounded-3xl bg-[var(--color-navy)] text-white overflow-hidden">
      <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--color-brand-yellow)] mb-2">{t('market_kicker')}</p>
        ...
        <Link href="/blog" className="text-sm font-semibold text-[var(--color-brand-yellow)] hover:underline">
          {t('view_all')}
        </Link>
      </div>
        ...
        {MARKET_INDEX.map((row) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-lg font-extrabold mt-2 text-[var(--color-brand-yellow)]">{formatTry(row.sale)}</p>
