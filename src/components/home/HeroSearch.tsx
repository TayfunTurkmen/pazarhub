...
export default function HeroSearch() {
  ...
  const selectCls =
    'w-full px-4 py-3 text-sm text-[var(--color-foreground)] bg-[var(--color-background)] rounded-xl border border-[var(--color-border)] focus:outline-none cursor-pointer';
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 max-w-full min-w-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {SEARCH_VERTICALS.map((item) => (
          <button
            ...
            className={`shrink-0 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-bold transition-colors ${
              vertical === item.id
      ...
      >
        ...
        <div className="lg:col-span-3 min-w-0">
          <LocationCascade
            ...
            showNeighborhood={vertical === 'emlak'}
            compact
