...
export default function SendeodePage() {
  const { user } = useAuth();
    ...
    <RouteGuard requireAuth>
      <div className="space-y-8">
        <section className="rounded-3xl bg-[var(--color-navy)] text-white px-6 py-10">
          <p className="text-[var(--color-brand-accent)] text-xs font-black uppercase tracking-[0.2em] mb-2">Sendeöde</p>
        ...
        <div className="space-y-3">
          {items.length === 0 && (
            <Card className="p-8 text-center text-sm text-[var(--color-muted)]">
              Henüz emanet işleminiz yok. Bir ilan sayfasından Sendeöde ile satın alın.
            </Card>
          )}
          {items.map((item) => {
