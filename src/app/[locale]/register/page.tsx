    ...
    return (
        <div className="max-w-md mx-auto mt-10">
            <Card className="p-8">
                ...
                <div className="mt-4 p-3 rounded-xl bg-[var(--color-navy)]/5 border border-[var(--color-navy)]/15 text-xs text-[var(--color-muted)]">
                    <p className="font-semibold text-[var(--color-foreground)] mb-1">{t('demo_accounts')}</p>
                    <p>{t('admin_hint')}</p>
                    <Link href="/login" className="inline-block mt-2 font-semibold text-[var(--color-navy)] dark:text-[var(--color-brand-yellow)] hover:underline">
                        {t('login_now')}
                    </Link>
                </div>
